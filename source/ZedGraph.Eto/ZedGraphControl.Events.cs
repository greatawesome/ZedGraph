...
namespace ZedGraph
{
	partial class ZedGraphControl
	{
		...
		public delegate void ContextMenuBuilderEventHandler( ZedGraphControl sender,
			ContextMenu menuStrip, Point mousePt, ContextMenuObjectState objState );
		/// <summary>
		...
		public event ZedMouseEventHandler ZMouseDownEvent;
		...
		/// </summary>
		[Bindable( false ), Browsable( false )]
		private new event EventHandler<MouseEventArgs> MouseMove;
		/// <summary>
		...
		public event ZedMouseEventHandler ZMouseUpEvent;
		/// <summary>
		...
		public event ZedMouseEventHandler ZMouseMoveEvent;
		...
		public event ZedMouseEventHandler ZDoubleClickEvent;
		...
		protected void ZedGraphControl_MouseDown( object sender, MouseEventArgs e )
		{
			...
			Point mousePt = Point.Round( new PointF( e.Location.X, e.Location.Y ) );
			...
			if (_masterPane != null && this.ZMouseDownEvent != null)
			{
				if (this.ZMouseDownEvent( this, e ))
					return;
			}
			...
			if (pane != null &&
					e.Buttons == _linkButtons && e.Modifiers == _linkModifierKeys)
			{
				object source;
				Link link;
				int index;
			...
			if (pane != null)
			{
				if (( _isEnableHPan || _isEnableVPan ) &&
					( ( e.Buttons == _panButtons && e.Modifiers == _panModifierKeys ) ||
					( e.Buttons == _panButtons2 && e.Modifiers == _panModifierKeys2 ) ))
				{
					BeginPan( mousePt, pane );
				}
				else if (_isEnableSelection && e.Buttons == _selectButtons &&
						( e.Modifiers == _selectModifierKeys || e.Modifiers == _selectAppendModifierKeys ))
				{
					BeginSelection( mousePt, pane );
				}
				else if (( IsEnableHCursorMove || IsEnableVCursorMove ) && e.Buttons == MouseButtons.Primary)
				{
					BeginMoveCursor( mousePt, pane );
				}
				else if (( _isEnableHEdit || _isEnableVEdit ) &&
						 ( e.Buttons == EditButtons && e.Modifiers == EditModifierKeys ))
				{
					BeginEdit( mousePt, pane );
				}
				else if (( _isEnableHZoom || _isEnableVZoom ) &&
					   ( ( e.Buttons == _zoomButtons && e.Modifiers == _zoomModifierKeys ) ||
					   ( e.Buttons == _zoomButtons2 && e.Modifiers == _zoomModifierKeys2 ) ))
				{
		...
		protected void SetCursor()
		{
			...
			SetCursor( new Point( (int)pf.X, (int)pf.Y ) );
		}
		...
		protected void SetCursor( Point mousePt )
		{
			if (_masterPane != null)
			{
				...
				if (( _isEnableHPan || _isEnableVPan ) && ( Keyboard.Modifiers == Keys.Shift || _isPanning ) &&
				...
				{
					this.Cursor = c.Orientation == CursorOrientation.Horizontal ? Cursors.HorizontalSplit : Cursors.VerticalSplit;
				}
		...
		protected void ZedGraphControl_MouseUp( object sender, MouseEventArgs e )
		{
			...
			if (_masterPane != null && this.ZMouseUpEvent != null)
			{
				if (this.ZMouseUpEvent( this, e ))
					return;
			}
		...
		protected void ZedGraphControl_MouseMove( object sender, MouseEventArgs e )
		{
			if (_masterPane != null)
			{
				...
				if (this.ZMouseMoveEvent != null && this.ZMouseMoveEvent( this, e ))
					return;
		...
		protected void ZedGraphControl_MouseWheel( object sender, MouseEventArgs e )
		{
			...
			{
				GraphPane pane = this.MasterPane.FindChartRect( new PointF( e.Location.X, e.Location.Y ) );
				if (pane != null && e.Delta.Height != 0)
				{
					...
					PointF centerPoint = new PointF( e.Location.X, e.Location.Y );
					double zoomFraction = ( 1 + ( e.Delta.Height < 0 ? 1.0 : -1.0 ) * ZoomStepFraction );
		...
		{
			//NOTE: This will need to be rewritten to work on multiple platforms, and to deal with the lack of ReversibleFrame
			...
			//ReversibleFrame.Draw(g, Color.Transparent, rect);
		...
		{
			var mousePtF = BoundPointToRect( Point.Round( new PointF( e.Location.X, e.Location.Y ) ), _dragPane.Chart.Rect );
		...
		private void HandleSelectionFinish( object sender, MouseEventArgs e )
		{
			...
			PointF mousePt = BoundPointToRect( Point.Round( new PointF( e.Location.X, e.Location.Y ) ), _dragPane.Rect );

			Eto.Drawing.PointF etoCurPt = ( (Control)sender ).PointToScreen( new Eto.Drawing.PointF( mousePt.X, mousePt.Y ) );

			Point curPt = Point.Round( new PointF( etoCurPt.X, etoCurPt.Y ) );
					  ...
					  ( Math.Abs( mousePtF.Y - _dragStartPt.Y ) > 4 ))
			{
				...
				Eto.Drawing.PointF etoStartPoint = ( (Control)sender ).PointToScreen( new Eto.Drawing.PointF( this._dragPane.Rect.X, this._dragPane.Rect.Y ) );

				PointF startPoint = new PointF( etoStartPoint.X, etoStartPoint.Y );
				...
				{
					if (g != null)
						_dragPane.FindContainedObjects( rF, g, out objects );
				}

				if (Keyboard.Modifiers == _selectAppendModifierKeys)
