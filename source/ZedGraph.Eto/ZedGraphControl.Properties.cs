...
namespace ZedGraph
{
	partial class ZedGraphControl
	{
		...
		[Bindable( true ), Category( "Display" ), NotifyParentProperty( true ),
		 DefaultValue( MouseButtons.Primary ),
		...
		[Bindable( true ), Category( "Display" ), NotifyParentProperty( true ),
		 DefaultValue( MouseButtons.Alternate ),
		...
		*/
		...
		public SaveFileDialog SaveFileDialog
		{
			get
			{
				if (_saveFileDialog == null)
					...
					{
						Filters = {
							new FileFilter("PNG Format", ".png"),
							new FileFilter("Gif Format", ".gif"),
							new FileFilter("Jpeg Format", ".jpg"),
							new FileFilter("Tiff Format", ".tiff"),
							new FileFilter("Bmp Format", ".bmp"),
						}
					};
		...
		[Bindable( false ), Browsable( false ),
		 DesignerSerializationVisibility( DesignerSerializationVisibility.Hidden )]
		public Eto.Drawing.Image GetImage()
		{
			//BDG TODO
			//Not the most useful call since we're inside an Eto ImageView, so you can just get this.Image instead...
			//You could aruge that this should just return this.Image rather than going to the masterpane
			...
			lock (this)
			{
				...
				return _masterPane.GetImage().ToEto();
			}
		}
