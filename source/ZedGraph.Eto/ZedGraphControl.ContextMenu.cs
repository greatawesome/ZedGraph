...
namespace ZedGraph
{
	partial class ZedGraphControl
	{
		...
		private ContextMenuObjectState GetObjectState()
		{
			...
			Eto.Drawing.PointF pf = this.PointFromScreen( Mouse.Position );
			PointF mousePt = new PointF( pf.X, pf.Y );
			int iPt;
			GraphPane pane;
			object nearestObj;
		...
		private void contextMenuStrip1_Opening( object sender, EventArgs e )
		{
			...
			//e.Cancel = true;
			ContextMenu menuStrip = sender as ContextMenu;
			...
			if (_masterPane != null && menuStrip != null)
			{
				...
				Eto.Drawing.PointF pf = this.PointFromScreen( Mouse.Position );
				_menuClickPt = new Point( (int)pf.X, (int)pf.Y );
				...
				if (_isShowContextMenu)
				{
					...
					MenuItem item = new ButtonMenuItem();
					//item.Name = "copy";
					item.Tag = "copy";
					item.Text = ZedGraphLocale.copy;
					item.Command = new Command( new System.EventHandler<EventArgs>( this.MenuClick_Copy ) );
					...
					item.Command = new Command( new System.EventHandler<EventArgs>( this.MenuClick_SaveAs ) );
					menuStrip.Items.Add( item );
					...
					//item.Command = new Command(new System.EventHandler<EventArgs>(this.MenuClick_PageSetup ));
					menuStrip.Items.Add( item );
					...
					//item.Command = new Command(new System.EventHandler<EventArgs>(this.MenuClick_Print ));
					menuStrip.Items.Add( item );
					...
					item.Command = new Command( new System.EventHandler<EventArgs>( this.MenuClick_ShowValues ) );
					( item as CheckMenuItem ).Checked = this.IsShowPointValues;
					menuStrip.Items.Add( item );
					...
					item.Text = menuStr;
					item.Command = new Command( new System.EventHandler<EventArgs>( this.MenuClick_ZoomOut ) );
					if (pane == null || pane.ZoomStack.IsEmpty)
						item.Enabled = false;
					menuStrip.Items.Add( item );
					...
					item.Text = menuStr;
					item.Command = new Command( new System.EventHandler<EventArgs>( this.MenuClick_ZoomOutAll ) );
					if (pane == null || pane.ZoomStack.IsEmpty)
						item.Enabled = false;
					menuStrip.Items.Add( item );
					...
					item.Text = menuStr;
					item.Command = new Command( new System.EventHandler<EventArgs>( this.MenuClick_RestoreScale ) );
					if (pane == null)
						item.Enabled = false;
					menuStrip.Items.Add( item );
		...
		{
			//ImageRenderer already using a memory stream in its ToEto method
			Clipboard.Instance.Image = ImageRender();

			/* The original code causes a crash in any backend other than Windows
			using (MemoryStream stream = new MemoryStream())
			{
				ImageRender().Save(stream, Eto.Drawing.ImageFormat.Bitmap);
				Clipboard.Instance.Image = new Eto.Drawing.Bitmap(stream);
			}
			*/
		}
		...
		/// </summary>
		/// <returns></returns>
		private Eto.Drawing.Bitmap ImageRender()
		{
			return _masterPane.GetImage( _masterPane.IsAntiAlias ).ToEto();
		}
		...
		//                ClipboardMetafileHelper.PutEnhMetafileOnClipboard(this.NativeHandle, metaFile);
		...
		/// </remarks>
		public void SaveAs( System.Object sender )
		{
			SaveAs( sender, null );
		}
		...
		/// which allow for Bitmap-only or Emf-only handling of the "Save As" context menu item.
		/// </remarks>
		public String SaveAs( System.Object sender, String DefaultFileName )
		{
			//NOTE: Only allow emf on MS Windows

			if (_masterPane != null)
			{
				//_saveFileDialog.CurrentFilter =
				//    //					"Emf Format (*.emf)|*.emf|" +
				//    "PNG Format (*.png)|*.png|" +
				//    "Gif Format (*.gif)|*.gif|" +
				//    "Jpeg Format (*.jpg)|*.jpg|" +
				...
				if (DefaultFileName != null && DefaultFileName.Length > 0)
				{
					...
					switch (ext)
					{
						case ".png": SaveFileDialog.CurrentFilterIndex = 1; break;
						case ".gif": SaveFileDialog.CurrentFilterIndex = 2; break;
						case ".jpeg":
						case ".jpg": SaveFileDialog.CurrentFilterIndex = 3; break;
						case ".tiff":
						case ".tif": SaveFileDialog.CurrentFilterIndex = 4; break;
						case ".bmp": SaveFileDialog.CurrentFilterIndex = 5; break;
					}
				...
				if (SaveFileDialog.ShowDialog( sender as Control ) == DialogResult.Ok)
				{
					Stream myStream = File.Open( SaveFileDialog.FileName, FileMode.Create );
					if (myStream != null)
					{
						Eto.Drawing.ImageFormat format = Eto.Drawing.ImageFormat.Png;
						switch (SaveFileDialog.CurrentFilterIndex)
						{
							case 1: format = Eto.Drawing.ImageFormat.Png; break;
							case 2: format = Eto.Drawing.ImageFormat.Gif; break;
							case 3: format = Eto.Drawing.ImageFormat.Jpeg; break;
							case 4: format = Eto.Drawing.ImageFormat.Tiff; break;
							case 5: format = Eto.Drawing.ImageFormat.Bitmap; break;
						}
		...
		/// </remarks>
		public void SaveAsBitmap( Object sender )
		{
			if (_masterPane != null)
			{
				if (SaveFileDialog.ShowDialog( sender as Control ) == DialogResult.Ok)
				{
					Eto.Drawing.ImageFormat format = Eto.Drawing.ImageFormat.Png;
					if (SaveFileDialog.CurrentFilterIndex == 2)
						format = Eto.Drawing.ImageFormat.Gif;
					else if (SaveFileDialog.CurrentFilterIndex == 3)
						format = Eto.Drawing.ImageFormat.Jpeg;
					else if (SaveFileDialog.CurrentFilterIndex == 4)
						format = Eto.Drawing.ImageFormat.Tiff;
					else if (SaveFileDialog.CurrentFilterIndex == 5)
						format = Eto.Drawing.ImageFormat.Bitmap;
		...
		/// <see cref="SaveAs()" />, which allows for Bitmap or EMF formats.
		/// </remarks>
		//public void SaveAsEmf(Object sender)
		//{
		//    if (_masterPane != null)
		//    {
		//        _saveFileDialog.CurrentFilter = "Emf Format (*.emf)|*.emf";

		//        if (_saveFileDialog.ShowDialog(sender as Control) == DialogResult.Ok)
		//        {
		//            Stream myStream = File.Open(_saveFileDialog.FileName, FileMode.Create);
		//            if (myStream != null)
		//            {
		//                myStream.Close();
		//                //_masterPane.GetMetafile().Save( _saveFileDialog.FileName );
		//                SaveEmfFile(sender, _saveFileDialog.FileName);
		//            }
		//        }
		//    }
		//}
		...
		/// <see cref="SaveAs()" />, which allows for Bitmap or EMF formats.
		/// </remarks>
		//internal void SaveEmfFile(object sender, string fileName)
		//{
		...
		//        ClipboardMetafileHelper.SaveEnhMetafileToFile(sender, metaFile, fileName);
		...
		//    static internal bool SaveEnhMetafileToFile(Object sender, Metafile mf, string fileName)
		//    {
		//        bool bResult = false;
		//        IntPtr hEMF;
		...
		//    static internal bool SaveEnhMetafileToFile(object sender, Metafile mf)
		//    {
		//        bool bResult = false;
		//        IntPtr hEMF;
		//        hEMF = mf.GetHenhmetafile(); // invalidates mf 
		...
		//            sfd.CurrentFilter = "Extended Metafile (*.emf)|*.emf";
		//            //sfd.DefaultExt = ".emf";
		//            if (sfd.ShowDialog(sender as Control) == DialogResult.Ok)
		//            {
